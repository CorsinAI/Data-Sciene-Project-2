"""
train_asl_citizen_transformer.py — Transformer encoder trained on ASL Citizen dataset.

Based on train_wlasl_transformer_aug.py (current best model) adapted for
ASL Citizen's CSV-based split structure.

Dataset: 83,399 videos across 2,731 glosses (~30 videos/gloss vs WLASL's ~6)
Splits : data/ASL_Citizen/splits/{train,val,test}.csv

Run from the project root:
    python -m src.training.train_asl_citizen_transformer
    python -m src.training.train_asl_citizen_transformer --eval-only
"""
...
KEYPOINT_DIR = PROJECT_ROOT / "data" / "processed" / "asl_citizen_keypoints_trimmed"
CHECKPOINT_DIR = PROJECT_ROOT / "checkpoints"
REPORT_DIR   = PROJECT_ROOT / "outputs" / "reports" / "asl_citizen_transformer"
PROCESSED_DIR = PROJECT_ROOT / "data" / "processed"

for d in [CHECKPOINT_DIR, REPORT_DIR, PROCESSED_DIR]:
    d.mkdir(parents=True, exist_ok=True)

BEST_MODEL_PATH    = CHECKPOINT_DIR / "asl_citizen_transformer.pt"
LABEL_MAP_PATH     = PROCESSED_DIR / "asl_citizen_label_mapping.csv"
HISTORY_PATH       = PROCESSED_DIR / "asl_citizen_train_history.csv"
CONFUSION_PATH     = REPORT_DIR / "confusion_matrix.csv"
PER_CLASS_PATH     = REPORT_DIR / "per_class.csv"
MISCLASSIFIED_PATH = REPORT_DIR / "misclassified.csv"
...
def load_split_records() -> tuple[list, list, list]:
    """
    Returns (train_recs, val_recs, test_recs) where each record is
    (video_stem, gloss, split).
    Only includes records that have a .npy file on disk.
    """
    def _load(csv_name, split_name):
        df  = pd.read_csv(SPLITS_DIR / csv_name)
        out = []
        for _, row in df.iterrows():
            stem = Path(row["Video file"]).stem
            gloss = str(row["Gloss"]).strip().upper()
            if (KEYPOINT_DIR / f"{stem}.npy").exists():
                out.append((stem, gloss, split_name))
        return out

    train_recs = _load("train.csv", "train")
    val_recs   = _load("val.csv",   "val")
    test_recs  = _load("test.csv",  "test")
    return train_recs, val_recs, test_recs


def build_label_map(train_recs, val_recs, test_recs) -> dict[str, int]:
    """Alphabetically sorted gloss → integer index (0-based)."""
    all_glosses = {gloss for recs in (train_recs, val_recs, test_recs)
                   for _, gloss, _ in recs}
    return {g: i for i, g in enumerate(sorted(all_glosses))}
...
class ASLCitizenDataset(Dataset):
    ...
    def __getitem__(self, idx):
        stem, gloss, _ = self.records[idx]
        label = self.label_map[gloss]
        raw   = np.load(KEYPOINT_DIR / f"{stem}.npy")   # (T, 225)
...
def main():
    ...
    print(f"Model  : Transformer  d_model={D_MODEL}  nhead={NHEAD}  "
          f"layers={NUM_LAYERS}  dim_ff={DIM_FF}  dropout={DROPOUT}")
    print(f"Aug    : speed [{SPEED_MIN}x–{SPEED_MAX}x]  noise={NOISE_STD}  "
          f"frame drop={FRAME_DROP_P}")

    print("Loading records...")
    train_recs, val_recs, test_recs = load_split_records()
    print(f"Train: {len(train_recs)}  Val: {len(val_recs)}  Test: {len(test_recs)}")

    if not train_recs:
        raise RuntimeError("No .npy files found. Run extract_keypoints_asl_citizen.py first.")

    label_map   = build_label_map(train_recs, val_recs, test_recs)
    ...
    train_loader = DataLoader(ASLCitizenDataset(train_recs, label_map, augment=True),
                              batch_size=BATCH_SIZE, shuffle=True,
                              num_workers=NUM_WORKERS, pin_memory=(DEVICE == "cuda"))
    val_loader   = DataLoader(ASLCitizenDataset(val_recs,   label_map, augment=False),
                              batch_size=BATCH_SIZE, shuffle=False, num_workers=NUM_WORKERS)
    test_loader  = DataLoader(ASLCitizenDataset(test_recs,  label_map, augment=False),
                              batch_size=BATCH_SIZE, shuffle=False, num_workers=NUM_WORKERS)
    ...
    pd.DataFrame(per_class).sort_values("recall").reset_index(
        drop=True).to_csv(PER_CLASS_PATH, index=False)

    misclassified = [
        {"video_stem": test_recs[i][0], "true_gloss": class_names[yt],
