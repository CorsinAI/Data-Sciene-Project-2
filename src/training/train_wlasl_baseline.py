...
REPORT_DIR = PROJECT_ROOT / "outputs" / "reports" / "wlasl"
CHECKPOINT_DIR = PROJECT_ROOT / "checkpoints"

for folder in [PROCESSED_DIR, REPORT_DIR, CHECKPOINT_DIR]:
    folder.mkdir(parents=True, exist_ok=True)

CSV_PATH = PROCESSED_DIR / "wlasl_model_metadata_min_frames_8.csv"

BEST_MODEL_PATH = CHECKPOINT_DIR / "best_wlasl_resnet18_top10_frames24.pt"
LABEL_MAP_PATH = PROCESSED_DIR / "wlasl_label_mapping_top10_frames24.csv"
HISTORY_PATH = REPORT_DIR / "wlasl_training_history_top10_frames24.csv"
CONFUSION_MATRIX_PATH = REPORT_DIR / "wlasl_confusion_matrix_top10_frames24.csv"
PER_CLASS_PATH = REPORT_DIR / "wlasl_per_class_performance_top10_frames24.csv"
CONFUSION_PAIRS_PATH = REPORT_DIR / "wlasl_confusion_pairs_top10_frames24.csv"
MISCLASSIFIED_PATH = REPORT_DIR / "wlasl_misclassified_samples_top10_frames24.csv"
ALL_PREDICTIONS_PATH = REPORT_DIR / "wlasl_all_test_predictions_top10_frames24.csv"
...
def main():
    ...
    required_cols = ["video_path", "gloss", "split"]
    ...
    df = df[df["gloss"].isin(top_classes)].copy()
    ...
    print(pd.crosstab(df["gloss"], df["split"]))

    print("\nRandom baseline:", 1 / num_classes)

    train_dataset = WLASLDataset(train_df, NUM_FRAMES, IMG_SIZE)
    val_dataset = WLASLDataset(val_df, NUM_FRAMES, IMG_SIZE)
    test_dataset = WLASLDataset(test_df, NUM_FRAMES, IMG_SIZE)
